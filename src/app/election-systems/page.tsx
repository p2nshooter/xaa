...
const COPY: Record<Lang, Copy> = {
  en: {
    ...
    scopeBody0: 'Ranges cover the honest span between adapting what we have already built and a programme with its own register format, its own legal obligations and its own integrations. The factors that move a figure inside its range: number of polling stations and field officers, the peak submission window, audit and chain-of-custody requirements, languages, accessibility obligations, whether a public results surface is in scope, and the SLA you need around election day. Final pricing follows a scope review, as with every ',
  ...
  es: {
    ...
    scopeBody0: 'Los rangos cubren el intervalo honesto entre adaptar lo que ya hemos construido y un programa con su propio formato de registro, sus propias obligaciones legales y sus propias integraciones. Los factores que mueven una cifra dentro de su rango: número de mesas y agentes de campo, la ventana de envío pico, requisitos de auditoría y cadena de custodia, idiomas, obligaciones de accesibilidad, si una superficie pública de resultados entra en el alcance, y el SLA que necesitas en torno al día electoral. El precio final sigue a una revisión del alcance, como con cada ',
